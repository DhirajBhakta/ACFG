...
def patternHALT(sentence):
	sentence.pattern = "HALT"

def markForDelete(sentence):
	sentence.pattern = ""

def patternFLAG(sentence):
	sentence.pattern = "FLAG"

def patternLOCK(sentence):
	sentence.pattern = "LOCK"

def patternSTACK(sentence):
	sentence.pattern = "STACK"

def patternJUMPSTACK(sentence):
	sentence.pattern = "JUMPSTACK"

def patternFLAGSTACK(sentence):
	sentence.pattern = "FLAGSTACK"

def patternASSIGN(sentence):
	sentence.pattern = "ASSIGN"

def patternASSIGNC(sentence):
	sentence.pattern = "ASSIGNC"

def patternLIBCALL(sentence):
	sentence.pattern = "LIBCALL"

def patternLIBCALLC(sentence):
	sentence.pattern = "LIBCALLC"

def patternCONTROL(sentence):
	sentence.pattern = "CONTROL"

def patternCONTROLC(sentence):
	sentence.pattern = "CONTROLC"

def patternASSIGN_OR_ASSIGNC_0(sentence):
	if (sentence.operands[0].startswith("0x")):
		sentence.pattern = "ASSIGNC"
	else:
		sentence.pattern = "ASSIGN"

def patternASSIGN_OR_ASSIGNC_1(sentence):
	if (sentence.operands[1].startswith("0x")):
		sentence.pattern = "ASSIGNC"
	else:
		sentence.pattern = "ASSIGN"


def patternASSIGN_OR_ASSIGNC_2(sentence):
	if (sentence.operands[2].startswith("0x")):
		sentence.pattern = "ASSIGNC"
	else:
		sentence.pattern = "ASSIGN"
...
def patternASSIGN_OR_ASSIGNC_all(sentence):
	const_operands = filter(lambda operand:operand.startswith("0x"), sentence.operands)
	if (len(const_operands) == 0):
		sentence.pattern = "ASSIGN"
	else:
		sentence.pattern = "ASSIGNC"

def patternASSIGN_OR_ASSIGNC_12(sentence):
	const_operands = filter(lambda operand:operand.startswith("0x"), sentence.operands[1:])
	if (len(const_operands) == 0):
		sentence.pattern = "ASSIGN"
	else:
		sentence.pattern = "ASSIGNC"

def patternASSIGN_OR_ASSIGNC_argnum(sentence):
	if len(sentence.operands)==3:
		sentence.patternASSIGN_OR_ASSIGNC_2()
	else:
		sentence.patternASSIGN_OR_ASSIGNC_1()


def patternJUMP_OR_JUMPC_0(sentence):
	if(sentence.operands[0].startswith("0x")):
		sentence.pattern = "JUMPC"
	else:
		sentence.pattern = "JUMP"

def patternSTACK_OR_STACKC_0(sentence):
	if(sentence.operands[0].startswith("0x")):
		sentence.pattern = "STACKC"
	else:
		sentence.pattern = "STACK"


def patternLIBCALL_OR_LIBCALLC_0(sentence):
	if(sentence.operands[0].startswith("0x")):
		sentence.pattern = "LIBCALLC"
	else:
		sentence.pattern = "LIBCALL"

def patternLIBCALL_OR_LIBCALLC_1(sentence):
	if(sentence.operands[1].startswith("0x")):
		sentence.pattern = "LIBCALLC"
	else:
		sentence.pattern = "LIBCALL"
...
def patternLIBCALL_OR_LIBCALLC_2(sentence):
	if(sentence.operands[2].startswith("0x")):
		sentence.pattern = "LIBCALLC"
	else:
		sentence.pattern = "LIBCALL"

def patternLIBCALL_OR_LIBCALLC_all(sentence):
	const_operands = filter(lambda operand:operand.startswith("0x"), sentence.operands)
	if (len(const_operands) == 0):
		sentence.pattern = "LIBCALL"
	else:
...
def patternLIBCALL_OR_LIBCALLC_12(sentence):
	const_operands = filter(lambda operand:operand.startswith("0x"), sentence.operands[1:])
	if (len(const_operands) == 0):
		sentence.pattern = "LIBCALL"
	else:
...
def patternCALL_OR_CALLC(sentence):
	const_operands = filter(lambda operand:operand.startswith("0x"), sentence.operands)
	if (len(const_operands) == 0):
		sentence.pattern = "CALL"
	else:
		sentence.pattern = "CALLC"

def patternCONTROL_OR_CONTROLC_all(sentence):
	const_operands = filter(lambda operand:operand.startswith("0x"), sentence.operands)
	if (len(const_operands) == 0):
		sentence.pattern = "CONTROL"
	else:
...
def patternTEST_OR_TESTC_all(sentence):
	const_operands = filter(lambda operand:operand.startswith("0x"), sentence.operands)
	if (len(const_operands) == 0):
		sentence.pattern = "TEST"
	else:
		sentence.pattern = "TESTC"

#-----****-----
def specialIMUL(sentence):
	if len(sentence.operands)==3:
		const_operands = filter(lambda operand:operand.startswith("0x"), sentence.operands[1:])
	else:
		const_operands = filter(lambda operand:operand.startswith("0x"), sentence.operands)
...
def specialJMP(sentence):
	idx = sentence.operands[0].find("0x")
	if (idx==0 or idx==1):
		sentence.pattern = "JUMPC"
	else:
		sentence.pattern = "JUMP"

def specialJxxx(sentence):
	idx = sentence.operands[0].find("0x")
	if (idx==0 or idx==1):
		sentence.pattern = "CONTROLC"
	else:
...
action ={
	...
	"DAS":markForDelete,
	"DEC":patternASSIGN,
	"DIV":patternASSIGN_OR_ASSIGNC_0,
	...
	"IDIV":patternASSIGN_OR_ASSIGNC_0,
	"IMUL":	specialIMUL,
	...
	"MUL":patternASSIGN_OR_ASSIGNC_0,
	"MWAIT":markForDelete,
	...
	"VPEXTRB":patternLIBCALL_OR_LIBCALLC_12,
	"VPEXTRD":patternLIBCALL_OR_LIBCALLC_12,
	"VPEXTRQ":patternLIBCALL_OR_LIBCALLC_12,
	"VPEXTRW":patternLIBCALL_OR_LIBCALLC_12,
